...
if not os.path.exists(temp_a):
    ...
    if (args.assembler == "megahit"): # MEGAHIT is OK with compressed input: .fastq inputted files contain .fastq.gz paths ,-delimited
        ...
        if (args.coassembly):
            ...
            mv_megahitCmd = 'mv '+out+'/final.contigs.fa '+out+'/temp_assembly.fa && gzip '+out+'/temp_assembly.fa'
    ...
    if args.assembler == "spades":
        ...
        if (args.coassembly): # non-gz file contains path list of all .gz inputs
            ...
            if '.gz' in read1_paths:
                ...
                read1_coa_tmp = out+'/'+ID+'.merged_1.fastq'
                read2_coa_tmp = out+'/'+ID+'.merged_2.fastq'

                if not os.path.isfile(read1_coa):
                    mergeCmd = 'cat '+read1_paths+' > '+read1_coa_tmp+' && cat '+read2_paths+' > '+read2_coa_tmp+' && gzip '+read1_coa+' '+read2_coa+''
                    subprocess.Popen(mergeCmd, shell=True).wait()
            ...
            mv_spadesCmd = 'mv '+out+'/scaffolds.fasta '+out+'/temp_assembly.fa && gzip '+out+'/temp_assembly.fa'
            subprocess.Popen(mv_spadesCmd, shell=True).wait()


        else: # individual assembly input is .fastq.gz
