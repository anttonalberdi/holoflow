    ...
    for CHR in chromosome_list:
        ...
        if not (chr_region == 'False'):
            ...
            if not (multicaller == 'False'):
                bcf1Cmd = 'module load bcftools/1.9 && bcftools mpileup -C '+degr_mqual+' -q '+min_mqual+' -Q '+min_bqual+' -Ou  -f '+ref_g+' -r '+CHR+' -b '+bam_list_file+' -r '+chr_region+' | bcftools call -m -v -Oz -o '+mpileup_output+''
                ...
                bcf1Cmd = 'module load bcftools/1.9 && bcftools mpileup -C '+degr_mqual+' -q '+min_mqual+' -Q '+min_bqual+' -Ou  -f '+ref_g+' -r '+CHR+' -b '+bam_list_file+' -r '+chr_region+' | bcftools call -v -Oz -o '+mpileup_output+''
                subprocess.Popen(bcf1Cmd,shell=True).wait()
                bcf2Cmd = 'bcftools view -m2 -M2 -v snps -Oz -o '+view_output+' '+mpileup_output+''
                subprocess.Popen(bcf2Cmd,shell=True).wait()
            ...
            if not (multicaller == 'False'):
                bcf1Cmd = 'module load bcftools/1.9 && bcftools mpileup -C '+degr_mqual+' -q '+min_mqual+' -Q '+min_bqual+' -Ou  -f '+ref_g+' -r '+CHR+' -b '+bam_list_file+' | bcftools call -m -v -Oz -o '+mpileup_output+''
                subprocess.Popen(bcf1Cmd,shell=True).wait()
                bcf2Cmd = 'bcftools view -m2 -M2 -v snps -Oz -o '+view_output+' '+mpileup_output+''
                subprocess.Popen(bcf2Cmd,shell=True).wait()

            else:
                bcf1Cmd = 'module load bcftools/1.9 && bcftools mpileup -C '+degr_mqual+' -q '+min_mqual+' -Q '+min_bqual+' -Ou  -f '+ref_g+' -r '+CHR+' -b '+bam_list_file+' | bcftools call -v -Oz -o '+mpileup_output+''
                subprocess.Popen(bcf1Cmd,shell=True).wait()
                bcf2Cmd = 'bcftools view -m2 -M2 -v snps -Oz -o '+view_output+' '+mpileup_output+''
                subprocess.Popen(bcf2Cmd,shell=True).wait()
