...
if not os.path.exists(out_dir):
    ...
    for i in range(len(mag_list)):
        ...
        for bam in bam_list:
            ...
            mag_counts_tmp = out_dir+'/'+mag_ID+'_'+sample+'.counts.txt'
            sample_list+=sample+'\t'

            if not os.path.isfile(new_bam):
            # Split bams into MAGs
            # Now BAM headers are only the contig ID - Removed MAG_ID-
                samtoolsCmd='module load tools samtools/1.11 && samtools view -h '+bam+' | grep "'+mag_ID+'-" | sed "s/'+mag_ID+'-//" | samtools view -bS - | htseq-count -t CDS -r pos -f bam - '+mag_annot+' > '+mag_counts_tmp+''
                subprocess.Popen(samtoolsCmd,shell=True).wait()

            else:
                htseqCountsCmd='module load tools && htseq-count -t CDS -r pos -f bam '+new_bam+' '+mag_annot+' > '+mag_counts_tmp+'' ## ?? --nonunique all ??
